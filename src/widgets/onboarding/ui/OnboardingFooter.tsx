...
export function OnboardingFooter() {
  ...
  const handleNext = () => {
    if (step === 3 && featureIndex < FEATURE_SLIDES_COUNT - 1) {
      nextFeature()
    } else {
      nextStep()
    }
  ...
  return (
    <>
      <DialogFooter
        className={`absolute right-0 bottom-0 m-0 flex w-full justify-between border-0 bg-transparent p-0 sm:justify-between ${step > 2 ? 'w-[36.8rem]' : 'w-full'}`}>
        {step <= 3 ? (
          <>
            <Button
              color={'gray'}
